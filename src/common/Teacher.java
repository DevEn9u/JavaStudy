...
public class Teacher extends Person {
	...
	/*
	 ...
	 처리하도록 hashCode와 equals 메서드를 적헐히 오버라이딩 하시오.
	 */
	/*
	 * hashCode()
	 : 인스턴스가 가진 고유한 값을 정수형으로 반환해준다.
	   따라서 서로 다른 인스턴스라면 다른 해시값을 가지게 된다.
	   정수형 멤버변수는 그냥 사용하면 되고, 인스턴스형 변수는
	   hashCode() 메서드를 통해 정수값으로 반환되는 해시값을 사용하면 된다.
	 */
	@Override
	public int hashCode() {
		/*
...
//		System.out.println("returnCode1 = " + returnCode1);
		 ...
		 * 방법2: 방법1이 번거롭다면 Objects 클래스의 정적메서드인 hash()를
		   사용해도 된다. 멤버변수의 개수만큼 인수로 전달하면 위와 동일하게 고유한
		   주소값을 정수형으로 반환해준다.
		 */
	...
	/*
	 * equals() 메서드
	 : 인스턴스가 가진 멤버변수의 값을 비교한다. 값이 같을 때는 true,
	   다를 때는 false를 반환한도록 오버라이딩 하면 된다.
	 */
	@Override
