...
class Friend implements Serializable {
	// 3개의 멤버변수: 이름, 전화번호, 주소 기본정보 3가지를 저장한다.
	String name;
	String phone;
	String addr;
...
class HighFriend extends Friend {
	...
	/*
	 * 오버라이딩1: 고등친구의 전체정보를 출력하기 위해 부모에서 정의한 메서드를
	 ...
	 * 오버라이딩2: 자식 클래스에서 오버라이딩하여 재정의한 메서드로
	   고등친구의 간략정보를 출력한다. 부모클래스에서는 실행부가 없는 상태로
	 * 정의되었다.
	 */
	@Override
	public void showBasicInfo() {
		System.out.println("==고등친구(간략정보)==");
		System.out.println("별명: " + nickname);
		System.out.println("전화번호: " + phone);
	}
}

// 대학 친구의 정보를 저장하기 위한 자식 클래스
...
public class E12MyFriendSerializable {
	...
	public static void main(String[] args) {
		// 사용자 입력을 위한 Scanner 인스턴스 생성
		Scanner scan = new Scanner(System.in);
		// 기능을 담당하는 핸들러 클래스의 인스턴스 생성
		FriendInfoHandler handler = new FriendInfoHandler(100);

		/*
		 * 프로그램을 시작하면 obj 파일에 있는 내용을 복원(역직렬화)해서
		   List에 추가한다.
		 */
		 ...
		 * 자주 사용된다. 
		 * for(;;)로도 무한루프를 구현할 수 있으나 반복의 횟수가 명확하지 않은
...
class FriendInfoHandler {
	...
	public void addFriend(int choice) {
		...
		/*
		 * List<E>는 자동 인덱싱이 지원되므로 인스턴스 추가를 위해 index변수를
	...
	// 전체 정보 출력 메서드
	public void showAllData() {
		/* 일반 for문을 상요하는 경우 변수 i를 인덱스로 사용해야 하므로
...
퀴즈2] 프로그램을 다시 시작하면 문제1에서 직렬화 했던 파일을 
	...
	//			}
	//			catch (IOException e) {
	//				System.out.println("원인 모를 에러 발생");
	//			}
	//		}
	//	}
	...
	public void saveFriendInfo() {
		try {
			...
			// List에 저장된 정보를 개수만큼 반복
			for(Friend f : lists) {
				/*
				 * List에 저장된 Friend 인스턴스를 순서대로 꺼내서
				   파일에 저장한다. 즉 직렬화한다.
				 */
				out.writeObject(f);
			}
	...
	// 프로그램이 시작되면 저장된 파일을 통해 복원 후 컬렉션에 추가한다. 
	public void readFriendInfo() {
		try {
			...
			while(true) {
				/*
				직렬화될떄 Object 기반으로 저장되므로 반드시 다운캐스팅해야 한다.
				 */
			...
			/*
			 * 예외가 발생하면 catach절로 예외인스턴스가 던져지므로 while 루프를
			   탈출할 수 있다.
			 */
			System.out.println("역직렬화중 예외 발생");
			// EOF(End of File)Exception 발생
			e.printStackTrace();
		}
