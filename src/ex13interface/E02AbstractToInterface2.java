...
class PersonalInfoVO {
	...
	/*
	 * getter/setter 메서드 정의: private로 정의된 멤버변수를 설정하거나
	   값을 반환받기 위해 생성함.
	 */
	...
	public void setJuminNum(String juminNume) {
		this.juminNum = juminNum;
	}
}
...
class PersonalNumberStorageImpl implements IPersonalNumberStorage {
	 ...
	 * 주민번호와 이름을 매개변수로 받은 후 VO인스턴스를 배열에 추가한다.
	 */
	@Override
	...
	public String searchPersonalInfo(String juminNum) {
		// 배열에 저장된 인스턴스의 개수만큼 반복
		for(int i = 0; i < numOfPerInfo; i++) {
			/*
			 * 매개변수로 전달된 주민번호를 통해 배열 검색
			 * compareTO(비교할 대상)메서드는 일치하면 0을 반환 
			 */
