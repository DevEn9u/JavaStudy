...
/*
 * else문에 조건식이 들어가면 에러가 발생하므로 주의해야 한다.
 */
public class E01If03 {
	...
	public static void main(String[] args) {
		 ...
		 * 90점 이상은 A학점, 60점 미만은 F학점을 출력
		 */
		// 점수는 정수타입으로 할당
		int kor = 99, eng = 70, math = 64;
		/*
		 * 평균값은 소수점이 나올 수 있으므로 실수형으로 선언하는 것이 좋다.
		 * 따라서 double의 결과를 얻기 위해 3.0(실수)으로 나눠준다.
		 */
		...
		System.out.println("잘못된 조건식의 if문에서는");
		/*
		 * 앞에서 작성한 if문에서 조건의 순서만 변경되었지만
		 * 60점 이상이라는 조건에 만족하기 때문에 높은 점수라도 모두 D학점이 출력됨.
		 * 문법적인 오류는 없으나 논리적 오류가 발생되었으므로 주의해야 한다.
		 */
		if(avg >= 60) {
			System.out.println("D학점");
		}
