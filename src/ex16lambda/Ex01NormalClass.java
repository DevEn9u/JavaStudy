...
// 1. 인터페이스와 클래스를 활요한 가장 일반적인 오버라이딩
/*
 * 이와 같이 추상클래스를 아래에 있는 인터페이스로 변경할 수 있다.
 * 인터페이스에 정의된 메서드는 public abstracat가 자동으로 추가된다.
	abstract class ISchool1 {
		abstract public void studyLambda(String str);
	}
 */
 ...
 * 추상메서드를 포함한 인터페이스를 구현하면 하위 클래스에서는 반드시
   오버라이딩 해야 한다. 추상메서드를 포함한 클래스는 반드시 abstract로
