...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		/*
		 아래 2개의 컬렉션은 내부적인 자료구조만 다를 뿐 구현방법에는
		 차이가 없다. 인스턴스 생성시 Student형의 타입 매개변수를
		 통해 생성한다. 즉, Student 인스턴스를 저장할 수 있는 상자가
		 된다.
		 */
...
//		ArrayList<Student> list2 = new ArrayList<Student>();
		...
		/*
		 ...
		 외부 패키지에 선언되었으므로 import 후 생성해야 한다.
		 */
		...
		list2.add(st3);
		list2.add(st4); // 인덱스 3
		// 리스트이므로 중복저장 가능, 인덱스로 구분할 수 있다.
		list2.add(st2);
		
		System.out.println("[출력1 일반 for문]");
		/*
		 * 일반 for문 사용을 위해 size() 메서드로 추가된 인스턴스의
		   개수를 반환받는다. 또한 index로 접귾내야 하므로 get()메서드를
		   사용한다.
		 */
		for(int i = 0; i < list2.size(); i++) {
			/*
			 List의 각 index에 저장된 인스턴스의 참조값을 그대로 출력한다.
		...
		/*
		 * 컬렉션에 저장된 값을 통해 Itreator 인스턴스를 생성한다.
		...
		for(Student st : list2) {
			/*
			 ...
			 getter를 통해 접근해야 한다. 삭제를 위해 입력한 이름과
			 Student 인스턴스에 저장된 name과 일치하는지 확인한 후
			 해당 인스턴스를 찾을 수 있다.
			 */
