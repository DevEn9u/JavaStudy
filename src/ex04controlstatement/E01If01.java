...
//		}
		...
		if(num%2==0 && num>=10) {
			System.out.printf("%n%d은/는 짝수이고 10보다 크거나 같다.%n", num);
		}
		...
		/*
		 * if(조건식); => 세미콜론은 문장의 끝을 나타래므로 조건에 상관없이 중괄호에 있는 문장이 실행되므로
		   주의해야 한다.
		 */
		if(num%2 != 0); {
		...
		if(num%2 == 0)
			System.out.printf("\n" + num + "은/는 짝수입니다. \n");
		if(num%2 != 0) {
		System.out.println("num은/는 " + num + "입니다.");
		...
		/*
		 * ASCII 코드를 알고있을 때의 구현방법
		 * 문자 0과 9가 ASCII code 48, 57에 해당하므로 구간 내에 있는지 확인할 수 있다.
		 */
		...
		/*
		 * ASCII 코드를 모를때 구현 방법
		 * 문자 '0'과 '9'는 메모리에 저장시 ASCII 코드로 변환되므로 아래와 같은 조건으로도 확인 가능.
		 */
