 ...
 * Java에서 제공하는 함수형 인터페이스에는 아래 4가지 정도가 있다.
   차이점이라면 매개변수와 리턴값의 유무이다.
   
 1) Predicate<T>: 매개변수 O, 리턴값 boolean -> 파라미터를 통해 true/false 반환
 2) Supplier<T>: 매개변수 X, 리턴값 O -> 실행후 호출한 곳으로 데이터를 return(공급)
 3) Consumer<T>: 매개변수 O, 리턴값 X -> 단순 출력 결과만 보여줌
 4) Function<T>: 매개변수 O, 리턴값 O -> 제일 많이 사용하는 유형
 */
public class Ex06Define1Predicate {
	...
	public static void main(String[] args) {
		 ...
		 * Predicate<T>
		 ...
		 성별이 '남'인 경우 true를 반환하도록 정의되어 있다.
		 */
		 ...
		 * Arrays.asList()는 배열을 List로 변환해주는 유틸리티 메서드이다.
		 */
	 ...
	 * 첫번째 매개변수는 성별이 '남'이면 true를 반환하는 람다식이고
	   두번째 개매변수는 4개의 Human 인스턴스가 저장된 List 컬렉션이다.
	 */
	public static double avg(Predicate<Human> ph, List<Human> li) {
		// 카운트용 변수
		int pCount = 0;
		int tCount = 0;
		// List에 저장된 Human 인스턴스의 개수만큼 반복(확장 for문)
		for(Human h : li) {
