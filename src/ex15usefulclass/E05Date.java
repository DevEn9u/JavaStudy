...
public class E05Date {
	...
	/*
	 * Calendar 클래스를 통해 얻은 인스턴스를 초로 반환한 후 매개변수로
	...
	public static void main(String[] args) {
		...
		/*
		 * Calendar 클래스는 대표적인 싱글톤 클래스이므로 new 키워드로
		   인스턴스를 생성할 수 없고, 유틸리티 메서드를 통해 참조값을 얻어와야 한다.
		 */
		...
		System.out.println(today + "에서 " + future + "까지 남은 날짜: " + result + "일");
	}
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			 ...
			 * getTime()으로 얻어오는 시간은 13자리인 정수값인데 10자리까지는
			   순수한 초(Sec)이고, 11~13자리는 ms를 의미한다.
			   즉 100으로 나눠서 뒤의 3자리를 제거하면 순수한 '초'가 된다.
			   또한 하루는 86,400초이므로 아래와 같이 남은 날짜를 계산할 수 있다.
			 */
