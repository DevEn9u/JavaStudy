...
/*
 * ArrayList<E>: List 계열의 collection
 - 데이터의 중복 저장이 허용된다.
 - 데이터의 저장 순서를 보장한다.
 - 데이터 접근시 get() 혹은 iterator()를 이용한다.
 - Array라는 이름처럼 '배열'의 특성을 가지고 있어 index를
   통한 저장 및 접근이 가능하다.
 */
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		/*
		 * String 인스턴스를 저장할 수 있는 List collection 생성
		   ArrayList와 나머지 컬렉션들은 데이터를 저장하는 내부적인
		   자료 구조만 다를 뿐 사용법은 100% 동일하다.
		 */
...
//		list.add(5, "소방차ㅋㅋ"); // 런타임 에러 발생
		...
		System.out.println("중복 저장된 객체수: " + list.size()); // 6

		/*
		 * 중복저장: List는 배열의 특성을 가지므로 동일한 값의 데이터를 중복저장할 수 있다.
		 		동일한 데이터라도 index로 구분할 수 있기 때문이다. add()는
		 		인스턴스 추가에 성공할 경우 true를 반환한다.
		 */
		System.out.println(list.add("트와이스") ? "중복저장됨" : "중복저장안됨");
		System.out.println("중복 저장 후 객체수: " + list.size()); // 7

		/*
		 * 컬렉션 출력시에는 아래 3가지 방법을 사용
		 * 방법1] 일반 for문 사용. 이때는 index를 통해 접근해야 하므로
		 		get()메서드를 사용한다.
		 */
		System.out.println("\n[일반 for문 사용]");
		...
		/*
		 * 방법2] 확장 for문의 경우 index값이 전체 원소를 반복해주므로
		 ...
		 * 삽입하기(끼워넣기): add(삽입할 인덱스, 인스턴스)
		 		해당 인덱스에인스턴스를 삽입한다. 기존 인스턴스는
		 		뒤로 1칸씩 밀려난다. 즉 인덱싱을 자동으로 해준다.
		 */
		...
		/*
		 * 포함여부 확인: contains(인스턴스)
		   		특정 인스턴스가 저장되어 있는지 확인할 때 상요한다.
		   		존재하면 true를 반환한다.
		 */
		 ...
		 /*
		  * 삭제 방법1]
		    remove(index): index를 통해 삭제를 진행한 후 삭제가 완료되면
		    	해당 인스턴스를 반환한다. 삭제 후에는 index가 자동으로
		    	부여된다(자동 인덱싱). 삭제 성공시 해당 인스턴스의 참조값을
		 ...
		 /*
		  * 삭제 방법2]
		    indexOf()를 통해 해당 인스턴스의 인덱스를 찾은 후 변수에 담아
		    remove()로 삭제한다.
		    index를 찾는 것을 제외하면 방법1과 동일하다.
		  */
		 ...
		 /*
		  * 삭제 방법3] 인스턴스의 참조값을 통해 삭제한다. 이 경우에는
		    삭제에 성공했을 때 true를 반환한다. 즉 boolean 타입의
		    반환값을 가진다.
		  * 
		  */
		 System.out.println(list.remove("방탄소년단") ? "삭제 성공(방탄)" : "삭제 실패(방탄)");
		 // 저장된 인스턴스가 없으므로 삭제에 실패한다.
		 System.out.println(list.remove("오마이걸") ? "삭제 성공(오마이걸)" : "삭제 실패(오마이걸)");
