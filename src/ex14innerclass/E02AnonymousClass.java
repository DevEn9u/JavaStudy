...
class Singer {
	 ...
	 * Object 클래스에 정의된 메서드로 객체가 가진 값을 문자열로
	   반환해준다. Java에서는 정의하는 모든 클래스는 Object클래스를
...
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		 ...
		 * 인스턴스의 참조값을 할당받은 참조변수를 출력한다. 이때 오버라이딩한
		   toSring()메서드의 반환값이 출력된다
		   만약 toString()을 삭제하면 @를 포함한 참조값이 출력된다.
		 */
		System.out.println(s1);
		
		// 자식클래스를 통해 인스턴스를 생성 후 정보 출력
		 ...
		 * Singer 타입의 변수에 GirlGroup 클래스를 상속받은 익명클래스를
		   생성해서 그 참조값을 할당한다.
		   개념적으로 new GirlGroup() {};
		 ...
		 * 부모의 참조변수로 자식을 참조할 수 있지만, 접근 범위가 부모까지로
		   제한되기 때문에 아래와 같이 자식에서 확장한 멤버변수는 접근할 수 없어
		   에러가 발생한다.
		   접근하고 싶다면 다운캐스팅(강제형변환)을 해야 하지만 익명클래스는
		   이름이 없기 때문에 다운캐스팅 자체가 불가능하다.
		   따라서 익명클래스에서는 멤버의 확장은 거의 사용되지 않는다.
		   대부분 오버라이딩을 목정으로 활용된다.
		 */
