 ...
 * 클래스가 상속관계에 있을 때 하위클래스에서 상위클래스의 멤버메서드와
   ...
   오버로딩과 다른 점은 매개변수의 개수 및 타입, 반환타입 등이 완전히 똑같은
   형태로 정의한다는 점이다.
   하위클래스에서 오버라이딩한 메서드는 상위클래스의 메서드를 가리게 되므로
   항상 하위클래스에서 오버라이딩한 메서드가 호출된다.
 */
...
class Speaker {
	...
	/*
	 * 멤버변수가 private으로 선언되었으므로 외부에서 값을 설정할 수 있도록
	   정의한 setter 메서드
	 */
...
class BaseSpeaker extends Speaker {
	...
	/*
	 * @Override: annotation이라 부르고, 오버라이딩된 메서드에 추가하여
	...
	public void showState() {
		 ...
		 * super를 통해 부모클래스에 정의된 멤버메서드를 호출한다.
		   특히 오버라이딩 된 메서드를 호출할 때 super가 없으면 재귀호출되어
		   무한루프에 빠질 수도 있으니 주의해야 한다.
		 */
		super.showState();
...
public class E06Overriding {
	...
	public static void main(String[] args) {
		 ...
		 * 단 부모타입의 변수로 자식 인스턴스를 참조하면 접근할 수 있는
		   범위가 부모 인스턴스까지로 제한되므로, 자식쪽에 정의된 메서드는
		   호출할 수 없다.
		 */
