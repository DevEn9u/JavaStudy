...
class Account {
	...
	/*
	 * 이와 같이 개발자가 생성자를 직접 정의하면 default 생성자는 추가되지 않는다.
	 * default 생성자는 개발자가 생성자를 정의하지 않았을 때만 컴파일러에 의해 자동으로 추가된다.
	 */
	...
	/*
	 * 현재는 부모와 자식 클래스가 동일한 패키지에 정의되어 있으므로 접근 지정자가
...
class SavingAccount extends Account {
	 ...
	 * 인자생성자에서 부모의 생성자를 호출하기 위한 super(x)를 사용한다.
	   ...
	   현재 부모클래스는 매개변수가 하나인 생성자 public Account(int init)가 유일하므로
	   만약 super(x)를 생략하면 에러가 발생한다. 
	 */
	public SavingAccount(int initVal) {
		super(initVal);
		/*
		 * 해당 문장을 삭제하면 즉시 에러 발생.
		   삭제하는 경우 super()라는 문장, 즉 매개변수가 없는 default 생성자를 호출하는
		   문장이 자동으로 삽입되는데 부모클래스에는 이런 생성자가 정의되지 않았으므로 에러가
		   발생한다.
		 */
...
//		sa.money = 100000; // 에러 발생
		...
		/*
		 * 부모클래스를 통해 Account 인스턴스를 생성한다. 하지만 해당 인스턴스 변수를
		   통해서도 money에 접근할 수 없다. 현재 접근하려는 위치가 클래스 외부이기 때문이다.
