...
public class E03StringBuilderBuffer {
	...
	public static void main(String[] args) {
		...
		// 문자열의 길이를 통해 마지막 부분에 'Z' 추가
		strBuf.insert(strBuf.length(), 'Z');
		System.out.println("strBuf= " + strBuf);
		System.out.println("==================");
		
		System.out.println("String과 StringBuffer의 참조값 비교");
		// double quotation으로 동일한 문자열을 생성하므로 동일한 주소값을 가짐
		String str1 = "Java&JSP";
		String str2 = "Java&JSP";
		if(str1 == str2) {
			System.out.println("연결전: 주소값 동일"); // 출력됨
		}
		else
			System.out.println("연결후: 주소값 다름");
		
		// String은 변경이 있는 경우 기준 메모리를 소멸한 후 새로운 메모리를 할당한다.
		System.out.println("### str1에 값을 변경 ###");
		str1 = str1 + "&Spring";
		if(str1 == str2) {
			System.out.println("연결전: 주소값 동일");
		}
		else
			System.out.println("연결후: 주소값 다름"); // 출력됨
