...
public class E03CallByValue {
	...
	public static void main(String[] args) {
		...
		// 출력1 : 100, 200
		System.out.println("[main메서드 안 - 호출 전]" +
				"first = " + first + ", second = " + second);
		// 메서드 호출시 인수로 값을 전달한다.
		callByValue(first, second);
		
		/* 
		 * 출력2: 매개변수로 전달된 값은 복사되어 다른 지역의 메서드 내에서만 변경되므로
		   main 메서드에서는 초기값이 그대로 유지된다.
		 */
		System.out.println("[main메서드 안 - 호출 후" +
				"first = " + first + ", second = " + second);
	}
	/*
	...
	public static void callByValue(int fNum, int sNum) {
		/*
		 * Swap
		 * 두 개의 변수가 할당받은 값을 서로 교환하는 알고리듬
		 * 교환시에는 임시변수(여기서는 temp)가 하나가 추가로 필요하다.
		 */
		int temp;
		temp = fNum;
		fNum = sNum;
		sNum = temp;
		/*
		 * 해당 메서드 내에서는 값의 교환이 완료되었으므로 출력값은 200, 100
		 */
		System.out.println("[callByValue 메서드 안"
