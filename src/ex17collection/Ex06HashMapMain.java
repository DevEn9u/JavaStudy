 ...
 * HashMap<K, V>: Map<T> 인터페이스를 구현한 컬렉션
 ...
 - Key값은 검색되므로 다른 컬렉션에 비해 속도가 빠르다.
 - 저장 순서는 유지되지 않는다.
 */
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		 ...
		 * 인스턴스 저장: 저장시 기존에 저장된 동일한 Key값이 존재하면
		   저장된 인스턴스가 반환된다. 만약 척음 입력이라면 null을 반환한다.
		 */
		// 처음 저장이므로 null이 반환된다.
...
//		map.put("age", number);
		...
		/*
		 * Value만 얻어와야 할 때는 values() 메서드를 통해 값을
		   인출할수 있다.
		   확장 fore문을 통해 출력한다.
		 */
		...
		System.out.println(map.containsKey("name") ?
				"name 키값 있음" : "name 키값 없음");
		System.out.println(map.containsKey("account") ?
				"account 키값 있음" : "account 키값 없음");
		...
		/*
		 * 인스턴스 삭제: Key를 통해 삭제하고, 삭제에 성공하면
		   해당 Value가 반환된다.
		 */
		System.out.println("삭제된 객체: " + map.remove("age")); // 20 출력
		System.out.println("[age 키값을 삭제한 후 출력]");
		for(String key : keys) {
			String value = map.get(key);
