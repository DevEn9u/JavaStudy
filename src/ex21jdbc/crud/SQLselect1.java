...
public class SQLselect1 extends MyConnection {
	...
	public void dbExecute() {
		try {
			...
			/*
			 정적 쿼리문 작성. to_char() 메서드를 통해 날짜의 서식을 지정한 컬럼은
			 별칭을 부여함.
			 */
			String query = "SELECT id, pass, name, regidate,"
					...
					+ " FROM member";
			/*
			행의 변화가 발생되지 않는 select 쿼리문은 executeQuery()를 호출하여
			실행한다. 조건에 맞는 레코드가 반환되므로 ResultSet 타입의 인스턴스를 반환값으로
			return된다.
			 */
			rs = stmt.executeQuery(query);
			/*
			 반환된 레코드의 개수만큼 반복한다. 조건에 따라 개수는 달라질 수 있으므로
			...
			while (rs.next()) {
				...
				// 컬럼을 지정할때는 인덱스 혹은 컬럼명 둘 다 사용할 수 있다.
				String pw = rs.getString("pass");
				String name = rs.getString("name");
				
				/*
				 오라클의 data 타입을 getDate()로 인출하면 0000-00-00 형식으로
				 출력된다. 또한 자료형이 Date이므로 java.sql 패키지의 클래스를 사용해야 한다.
				 */
				java.sql.Date regidate = rs.getDate("regidate");
				/*
				 날짜를 getString()으로 인출하면 시간까지 포함되서 출력된다.
				 yyyy-mm-dd hh:mm:ss 형식이므로 적당한 크릭도 자르고 시ㅠㅍ다면
				 문자열 처리 메서드를 사용하면 된다.
				 */
				...
				/*
				 오라클의 변환함수를 통해 출력하고자 하는 형태로 formatting한 후 문자형식으로
