...
public class E07RideAndLoad02 {
	...
	public static void main(String[] args) {
		...
		// 자식 인스턴스를 자식 참조변수(Dechild)로 참조(동일한 타입) 
		DeChild child = new DeChild("율곡이이", 49, "00학번");
		
		/*
		 * 오버라이딩 처리된 메서드이므로 참조변수와 상관 없이 항상
		   자식쪽 메서드(Dechild)가 호출된다. 메서드 오버라이딩은 자식쪽에
		   정의한 메서드가 부모쪽에 정의된 메서드를 가리게 된다.
		   따라서 오버라이딩된 메서드를 호출할 때는 참조변수의 영향을
		   받지 않는다.
		 */
		child.exercise();
		child.sleep();
		 ...
		 * 인스턴스 내부가 아닌 외부(Method 영역)에 이미 로드되어 있는
		   상태이기 때문이다.
		 */
		 ...
		 * 상속관계이므로 가능하다.
		 */
		System.err.println("Parent형 참조변수로 Child 객체 참조");
		DeParent parent = new DeChild("퇴계이황", 35, "99학번");
		
		/*
		 * 오버라이딩 처리된 메서드이므로 참주변수에 상관없이 항상 자식쪽의
		   메서드가 호출된다.
