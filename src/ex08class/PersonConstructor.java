...
public class PersonConstructor {
	 ...
	 * 개발자가 생성자를 직접 정의하지 않으면 아래와 같이 매개변수와 실행부가 없는
	   'default 생성자'가 컴파일러에 의해 자동으로 삽입된다.
	   지금까지 인스턴스 생성시 'new 클래스명()'과 같이 사용했던 문장이 바로...
	   'default 생성자'를 호출하는 것이었다.
	 */
	...
	// 생성자2: 매개변수 한 개가 있는 형태로 정의
	public PersonConstructor(String name) {
		/*
		 * 해당 this는 멤버변수와 매개변수를 구분하기 위한 용도로 사용된다.
		   클래스 자신, 즉 멤버를 가리킨다.
		   따라서 좌측항은 멤버변수, 우측항은 매개변수가 된다.
		   멤버변수 앞에 this를 붙이는 것이 원칙이나 붙이지 않아도 된다.
		   여기서는 this.name = name; 했을 때 좌측항과 우측항, 즉 멤버변수와 매개변수의
		   색이 다른 것을 확인할 수 있다.
		 */
		this.name = name;
		age = 1;
	...
	public PersonConstructor(String _name, int age, String addr) {
		// 매개변수와 멤버변수명이 다르면 구분이 되기 때문에 this를 사용하지 않아도 된다.
		name = _name;
	...
	/*
	 * 일반적인 멤버메서드: 멤버변수 초기화를 모적ㅇ으로 정의
	 * void로 선언하고 함수명도 소문자로 시작한다.
	 */
	
	void initialize(String name, int age, String addr) {
		/*
		 * 해당 코드는 생성자 내에서만 사용할 수 있다.
		 * 생성자는 인스턴스 생성시 딱 한 번만 호출되고 개발자가 임의로 호출할 수 없다.
