...
/*
 * 배열
   순차적인 메모리의 공간이 할당되는 형태로, 하나가 아니라 여러개의 변수가 필요한 경우에 사용한다.
 * Java에서는 배열을 인스턴스 형태로 생성하므로 new 키워드를 사용한다.
 * 생성된 배열을 Heap 메모리 공간에 저장되고 할당된 메모리의 주소값을 반환한다.
 * 따라서 배열명(배열변수)는 주소값(참조값)을 할당받게 된다.
 * 우리는 해당 주소값을 통해 배열에 접근이 가능하다.
 */
public class E01OneDimArray01 {
	...
	public static void main(String[] args) {
		...
		/*
		 * 인스턴스 배열
		 * 기본 자료형을 저장하는 배열이 아닌 인스턴스의 참조값을 저장할 목적으로 생성한 배열
		 * 사용법은 일반 배열과 동일하다.
		 */
		...
		/*
		 * 배열을 선언과 동시에 초기화하는 경우 자바 컴파일러는 초기값의 개수를 통해
		   배열의 크기를 결정할 수 있다. 따라서 이 경우에는 크기가 생략하도록 약속되어 있다.
		 */
