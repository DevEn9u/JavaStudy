...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		/*
		 * Object 기반으로 set 컬렉션을 생성했으므로 모든 인스턴스를
		   저장할 수 있다.
		 */
		HashSet<Object> set = new HashSet<Object>();
		/*
		 * 아래 2개의 문장도 위와 동일한 인스턴스 생성 문장으로
		   사용할 수 있지만, 타입매개변수를 쓰지 않는 것은 권장 x
		 */
...
//		HashSet<Object> set = new HashSet<>();
		...
		// size()를 통해 저장된 인스턴스의 개수를 출력: 5개 출력됨
		System.out.println("[중복 저장된 객체수]: " + set.size());
		/*
		 * 기본클래스(여기서는 String)의 인스턴스 중복 저장
		...
		/*
		 * Iterator를 통한 인스턴스 출력
		 : 순서 없이 저장되므로 출력의 순서도 정할 수 없다.
		 */
		// 1. set 참조변수를 통해 Itreator 인스턴스를 생성한다.
		...
		while(itr.hasNext()) {
			 ...
			 * HashSet<E>에 저장시 모두 Object형으로 자동형변환되어 저장되므로
			   우리가 직접 정의한 크래스에 대해서만 instanceof를 통해 타입을 확인한 후
			   다운캐스팅하여 출력한다. 만약 다운캐스팅을 하지 않으면 자식멤버에 
