 ...
 * 독립쓰레드(Non Daemon Thread)
 : 메인 쓰레드와 Working쓰레드(개발자가 정의한 쓰레드)를 일컫는 것으로
   메인 쓰레드가 종료되어도 계속 실행되며, Dead 상태가 되어야만 실행이
   종료되는 쓰레드를 말한다. 해당 쓰레드는 main() 메서드가 종료된 이후에도
   계속 실행되는 것을 볼 수 있다.
 */
class NormalThread extends Thread {
	/*
	 * 기본 생성자를 통해 인스턴스를 생성하면 쓰레드명이 JVM에 의해
	   자동으로 부여된다.
	 */
	public NormalThread() {}
	/*
	 ...
	 * 쓰레드 동작중 비정상적으로 종료될 때 InterruptedException이
	   ...
	   throws해서 호출한 위치로 예외인스턴스를 던지기 한다.
	 */
	...
	public void run() {
		try {
			/*
			 * 해당 메서드에서 무싲한 예외를 호출한 지점에서 처리한다.
 ...
 * 종속쓰레드(Daemon Thread)
 : 모든 독립쓰레드가 종료되면 자동으로 종료되는 쓰레드를 말한다.
   ...
   프로그램이 종료될때 함께 종료되므로 주로 무한루프로 구성된다.
 */
class DaemonThread extends Thread {
	/*
	 * 무한루프로 구성되어있고 별도의 탈출조건을 만들지 않는다.
	   3초에 한번씩 block상태로 들어가게 구현되어 있다.
	 */
	@Override
	public void run() {
		while(true) {
...
public class Ex03DaemonThread {
	 ...
	 * 쓰레드로 구현하지 않은 일반적인 클래스의 작업으로 순차적으로
	   진헁되어 1번 인스턴스가 작업을 마쳐야만 2번째가 실행된다.
	 */
	public static void main(String[] args) {
		...
		// 방법2: setName() 메서드를 통해 쓰레드의 이름 부여
		// yt1.setName("First 쓰레드");
		yt1.start();
		
		/*
		 * 두번째 독립쓰레드 생성 및 시작. 쓰레드명을 부여하지 않았으므로
		   JVM이 작동으로 이름을 부여한다. 우선순위는 10으로 지정한다.
		 */
		...
		/*
		 * Thread 클래스의 정적 메서드를 통해 쓰레드의 여러상태를 확인할 수 있다.
		 */
